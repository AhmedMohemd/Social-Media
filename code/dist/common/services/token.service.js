...
class TokenService {
    ...
    sign = async ({ payload, secret = config_1.ACCESS_USER_TOKEN_SIGNATURE, options, }) => {
        return jsonwebtoken_1.default.sign(payload, secret, options);
    };
    verify = async ({ token, secret = config_1.ACCESS_USER_TOKEN_SIGNATURE, options, }) => {
    ...
    detectSignatureLevel = async (role) => {
        ...
        switch (role) {
            case enums_1.RoleEnum.ADMIN:
                signatures = {
                    accessSignature: config_1.ACCESS_SYSTEM_TOKEN_SIGNATURE,
                    refreshSignature: config_1.REFRESH_SYSTEM_TOKEN_SIGNATURE,
                };
                break;
            default:
                signatures = {
                    accessSignature: config_1.ACCESS_USER_TOKEN_SIGNATURE,
                    refreshSignature: config_1.REFRESH_USER_TOKEN_SIGNATURE,
                };
                break;
        }
        return signatures;
