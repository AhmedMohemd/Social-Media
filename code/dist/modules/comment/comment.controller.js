...
router.post("/:commentId/reply", (0, middleware_1.authentication)(), (0, multer_1.cloudFileUpload)({ validation: multer_1.fileFieldValidation.image }).array("attachments", 2), (0, middleware_1.validation)(validators.replyOnComment), async (req, res, next) => {
    const data = await comment_service_1.commentService.replyOnComment(req.params, { ...req.body, files: req.files }, req.user);
    return (0, response_1.successResponse)({ res, status: 201, data });
});
router.patch("/:commentId/react", (0, middleware_1.authentication)(), (0, middleware_1.validation)(validators.reactComment), async (req, res) => {
